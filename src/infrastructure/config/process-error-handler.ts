...
export function registerProcessErrorHandlers(reporter: IErrorReporter, logger: ILogger): void {
  process.on('uncaughtException', (error: Error) => {
    logger.error('uncaughtException — process will exit', { message: error.message, stack: error.stack });
  ...
  process.on('unhandledRejection', (reason: unknown) => {
    ...
    logger.error('unhandledRejection — process will exit', { message: error.message, stack: error.stack });
